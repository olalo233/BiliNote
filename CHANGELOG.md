...
- 归档视频回退为取源站最佳编码（YouTube 高分辨率通常为 av1，压缩比优于 h264），不再强制 h264。
  播放黑屏的真实原因是 https 页面加载 http presigned URL 的 mixed content，已由 s3.expii.top
  HTTPS 入口解决；现代浏览器可解 av1-in-mp4，无需牺牲归档体积/分辨率换兼容。
...
- fork `olalo233/BiliNote` 的 `-ex` 扩展发行版：新增 MinIO / S3 对象存储支持。
- 图床：笔记截图可上传到对象存储，并在 Markdown 中使用公网绝对 URL。
- 资产归档：异步归档转写、字幕、音频和可选原始视频，支持恢复到本地资源目录。
- 资源包：按平台和视频列出归档对象，支持预签名访问、下载、删除和手动归档。
- 存储用量：设置页汇总对象数、总大小、最近上传时间，以及资产按类型的明细。
- 设置页按图床 / 资产分别管理供应商列表和功能详情，供应商类型限定为 MinIO / S3。
